...
@function
def get_duration_distributions(heartrate=None,systolic_time=None,*args,**kwargs):
    varargin = get_duration_distributions.varargin
    nargin = get_duration_distributions.nargin

    springer_options=copy(default_Springer_HSMM_options)

    mean_S1=round(dot(0.122,springer_options.audio_segmentation_Fs))

    std_S1=round(dot(0.022,springer_options.audio_segmentation_Fs))

    mean_S2=round(dot(0.094,springer_options.audio_segmentation_Fs))

    std_S2=round(dot(0.022,springer_options.audio_segmentation_Fs))

    mean_systole=round(dot(systolic_time,springer_options.audio_segmentation_Fs)) - mean_S1

    std_systole=dot((25 / 1000),springer_options.audio_segmentation_Fs)

    mean_diastole=dot(((60 / heartrate) - systolic_time - 0.094),springer_options.audio_segmentation_Fs)

    std_diastole=dot(0.07,mean_diastole) + dot((6 / 1000),springer_options.audio_segmentation_Fs)
    ...
    min_systole=mean_systole - dot(3,(std_systole + std_S1))

    max_systole=mean_systole + dot(3,(std_systole + std_S1))

    min_diastole=mean_diastole - dot(3,std_diastole)

    max_diastole=mean_diastole + dot(3,std_diastole)
