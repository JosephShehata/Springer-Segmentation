...
@function
def butterworth_high_pass_filter(original_signal=None,order=None,cutoff=None,sampling_frequency=None,figures=None,*args,**kwargs):
    varargin = butterworth_high_pass_filter.varargin
    nargin = butterworth_high_pass_filter.nargin

    if nargin < 5:
        figures=0
    
    #Get the butterworth filter coefficients
    B_high,A_high=butter(order,dot(2,cutoff) / sampling_frequency,'high',nargout=2)
...
#coefficients, ensuring zero phase distortion
    ...
    if (figures):
        ...
        sos,g=zp2sos(B_high,A_high,1,nargout=2)
