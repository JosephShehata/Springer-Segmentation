...
@function
def butterworth_low_pass_filter(original_signal=None,order=None,cutoff=None,sampling_frequency=None,figures=None,*args,**kwargs):
    varargin = butterworth_low_pass_filter.varargin
    nargin = butterworth_low_pass_filter.nargin

    if nargin < 5:
        figures=0
    
    #Get the butterworth filter coefficients
    B_low,A_low=butter(order,dot(2,cutoff) / sampling_frequency,'low',nargout=2)

    if (figures):
        figure('Name','Low-pass filter frequency response')
        sos,g=zp2sos(B_low,A_low,1,nargout=2)
