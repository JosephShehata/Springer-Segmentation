...
@function
def getHeartRateSchmidt(audio_data=None,Fs=None,figures=None,*args,**kwargs):
    varargin = getHeartRateSchmidt.varargin
    nargin = getHeartRateSchmidt.nargin
...
# This is performed after filtering and spike removal:
    ...
    signal_autocorrelation=c(arange(length(homomorphic_envelope) + 1,end()))

    min_index=dot(0.5,Fs)

    max_index=dot(2,Fs)

    __,index=max(signal_autocorrelation(arange(min_index,max_index)),nargout=2)
...
# cycle duration"
    ...
    max_sys_duration=round((dot((60 / heartRate),Fs)) / 2)

    min_sys_duration=round(dot(0.2,Fs))

    __,pos=max(signal_autocorrelation(arange(min_sys_duration,max_sys_duration)),nargout=2)
