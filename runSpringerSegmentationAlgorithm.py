...
@function
def runSpringerSegmentationAlgorithm(audio_data=None,Fs=None,B_matrix=None,pi_vector=None,total_observation_distribution=None,figures=None,*args,**kwargs):
    varargin = runSpringerSegmentationAlgorithm.varargin
    nargin = runSpringerSegmentationAlgorithm.nargin
    ...
    PCG_Features,featuresFs=getSpringerPCGFeatures(audio_data,Fs,nargout=2)

    ## Get PCG heart rate
    
    heartRate,systolicTimeInterval=getHeartRateSchmidt(audio_data,Fs,nargout=2)

    __,__,qt=viterbiDecodePCG_Springer(PCG_Features,pi_vector,B_matrix,total_observation_distribution,heartRate,systolicTimeInterval,featuresFs,nargout=3)
