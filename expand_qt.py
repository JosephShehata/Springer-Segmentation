...
@function
def expand_qt(original_qt=None,old_fs=None,new_fs=None,new_length=None,*args,**kwargs):
    varargin = expand_qt.varargin
    nargin = expand_qt.nargin

    original_qt=ravel(original_qt).T

    expanded_qt=zeros(new_length,1)

    indeces_of_changes=find(diff(original_qt))

    indeces_of_changes=concat([indeces_of_changes,length(original_qt)])

    start_index=0

    for i in arange(1,length(indeces_of_changes)).reshape(-1):
        start_index
        ...
        expanded_start_index=round(multiply((start_index / old_fs),new_fs)) + 1

        expanded_end_index=round(multiply((end_index / (old_fs)),new_fs))

        if (expanded_end_index > new_length):
            expanded_end_index=copy(new_length)

        expanded_qt[arange(expanded_start_index,expanded_end_index)]=value_at_mid_point

        start_index=copy(end_index)
