...
@function
def Homomorphic_Envelope_with_Hilbert(input_signal=None,sampling_frequency=None,lpf_frequency=None,figures=None,*args,**kwargs):
    varargin = Homomorphic_Envelope_with_Hilbert.varargin
    nargin = Homomorphic_Envelope_with_Hilbert.nargin
    ...
    B_low,A_low=butter(1,dot(2,lpf_frequency) / sampling_frequency,'low',nargout=2)

    homomorphic_envelope=exp(filtfilt(B_low,A_low,log(abs(hilbert(input_signal)))))

    # Remove spurious spikes in first sample:
    homomorphic_envelope[1]=concat([homomorphic_envelope(2)])
