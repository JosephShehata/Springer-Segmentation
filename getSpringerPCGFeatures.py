...
# in default_Springer_HSMM_options.py
...
@function
def getSpringerPCGFeatures(audio_data=None,Fs=None,figures=None,*args,**kwargs):
    varargin = getSpringerPCGFeatures.varargin
    nargin = getSpringerPCGFeatures.nargin
...
# Get the features used in the Springer segmentation algorithm.
    ...
    psd=get_PSD_feature_Springer_HMM(audio_data,Fs,40,60).T
    ...
    if (include_wavelet):
        ...
        if (length(audio_data) < dot(Fs,1.025)):
            audio_data=concat([[audio_data],[zeros(round(dot(0.025,Fs)),1)]])

        cD,cA=getDWT(audio_data,wavelet_level,wavelet_name,nargout=2)

        wavelet_feature=abs(cD(wavelet_level,arange()))

        wavelet_feature=wavelet_feature(arange(1,length(homomorphic_envelope)))

        downsampled_wavelet=resample(wavelet_feature,featuresFs,Fs)

        downsampled_wavelet=normalise_signal(downsampled_wavelet).T

    
    ##
    
    if (include_wavelet):
        PCG_Features=concat([downsampled_homomorphic_envelope,downsampled_hilbert_envelope,psd,downsampled_wavelet])

    else:
        PCG_Features=concat([downsampled_homomorphic_envelope,downsampled_hilbert_envelope,psd])

    
    ## Plotting figures
    if (figures):
        figure('Name','PCG features')
        t1=(arange(1,length(audio_data))) / Fs

        plot(t1,audio_data)
        hold('on')
        t2=(arange(1,length(PCG_Features))) / featuresFs
